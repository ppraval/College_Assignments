/* sim-inorder.c - sample in-order issue perf simulator implementation */
...
/*
 * This file implements a very detailed in-order issue superscalar
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_flag(odb, "-issue:wrongpath",
	       "issue instructions down wrong execution paths",
	       &ruu_include_spec, /* default */FALSE,
	       /* print */TRUE, /* format */NULL);
...
ruu_writeback(void)
{
  ...
  while ((rs = eventq_next_event()))
	...
	panic("inst completed and !ready, !issued, or completed");
      printf("WRITEBACK: inst=0x%08x, opcode=%s, PC=0x%08lx\n", rs->IR, MD_OP_NAME(rs->op), rs->PC);
      ...
      if (rs->recover_inst)
	{
	  ...
	  fetch_delay = ruu_branch_penalty;
...
printf("ISSUE: inst=0x%08x, PC=0x%08lx\n", rs->IR, rs->PC);
...
ruu_dispatch(void)
{
  ...
  while (/* instruction decode B/W left? */
	...
	//printf("Dispatching instruction #%llu, opcode: %s\n", (unsigned long long)sim_total_insn, MD_OP_NAME(op));
printf("DISPATCH: inst=0x%08x, opcode=%s, PC=0x%08lx\n", inst, MD_OP_NAME(op), regs.regs_PC);	
...
ruu_fetch(void)
{
  ...
  for (i=0, branch_cnt=0;
      ...
      else
	{
      ...
      printf("\nFetch Stage Inst - 0x%llx , PC -  0x%llx\n ",inst,fetch_regs_PC );	
      /* for pipe trace */
...
struct pipeline_buffer_t {
  int valid;              /* Is the latch holding a valid instruction? */
  md_inst_t IR;
  enum md_opcode op;
  md_addr_t PC;
  int out1, out2;         /* Destination registers */
  md_addr_t next_PC;      /* For branches */
  md_addr_t pred_PC;      /* Predicted next PC */
  /* Fields for operand values, results, etc. */
  md_gpr_t op_data;      /* Holds source operand values */
  md_gpr_t result_data;  /* Holds result to be written back */
  tick_t ready_cycle;     /* Cycle when the instruction completes this stage */
  struct bpred_update_t dir_update;/* Branch predictor update info */
};

static struct pipeline_buffer_t ID_EX_buffer;
static struct pipeline_buffer_t EX_MEM_buffer;
static struct pipeline_buffer_t MEM_WB_buffer;

static tick_t reg_ready_at[MD_TOTAL_REGS];

static void writeback_stage(void)
{
  /*writes the result of the instruction to the destination register specific to the instruction
  - The data being written back. once this stage is complete, the instruction is fully retired */
    if (!MEM_WB_buffer.valid)
  {
    return; /* Idle */
  }

  if (MEM_WB_buffer.ready_cycle > sim_cycle)
  {
    return; /* Wait for memory/execute to complete */
  }

  /* Write result back to architectural register file */
  if (MEM_WB_buffer.out1 != NA)
  {
    regs.regs_R[MEM_WB_buffer.out1] = MEM_WB_buffer.result_data[0];
  }
  if (MEM_WB_buffer.out2 != NA)
  {
    regs.regs_R[MEM_WB_buffer.out2] = MEM_WB_buffer.result_data[1];
  }
 if (pred
	  && bpred_spec_update == spec_CT
	  && (MD_OP_FLAGS(MEM_WB_buffer.op) & F_CTRL))
	{
	  bpred_update(pred,
		       /* branch address */MEM_WB_buffer.PC,
		       /* actual target address */MEM_WB_buffer.next_PC,
                       /* taken? */MEM_WB_buffer.next_PC != (MEM_WB_buffer.PC +
                                                   sizeof(md_inst_t)),
                       /* pred taken? */MEM_WB_buffer.pred_PC != (MEM_WB_buffer.PC +
                                                        sizeof(md_inst_t)),
                       /* correct pred? */MEM_WB_buffer.pred_PC == MEM_WB_buffer.next_PC,
                       /* opcode */MEM_WB_buffer.op,
                       /* dir predictor update pointer */&MEM_WB_buffer .dir_update);
	}
  /* Instruction is now complete and can be counted */
  if (!spec_mode)
    sim_num_insn++;

  MEM_WB_buffer.valid = FALSE; /* Free up the buffer */
}

static void memory_stage(void)
{
  /* Functionality
  - for load instructions: takes memory addr calculated in the execute stage and reads the corresponding data from the data cache or main memory
  - for store it takes the memory addr from execute stage and data from the decode stage and writes the data to the data cache or main memory
  - All other instruction: This stage is idle and simply passes the result to the next stage*/
    int i, load_lat, tlb_lat, n_issued;
  ...
  if (!EX_MEM_buffer.valid)
  {
    MEM_WB_buffer.valid = FALSE;
    return;
  }

  if (EX_MEM_buffer.ready_cycle > sim_cycle)
  {
    return; /* Wait for Execute stage to finish */
  }

  int lat = 0;
  if (MD_OP_FLAGS(EX_MEM_buffer.op) & F_MEM)
  {
    enum mem_cmd cmd = (MD_OP_FLAGS(EX_MEM_buffer.op) & F_STORE) ? Write : Read;
    md_addr_t addr = EX_MEM_buffer.result_data[0]; /* Assume address is in result_data */
    
    /* Access data cache */
    lat = cache_access(cache_dl1, cmd, addr, 
                      (cmd == Read) ? &MEM_WB_buffer.result_data[0] : &EX_MEM_buffer.op_data[0], 
                      sizeof(word_t), sim_cycle, NULL, NULL);
  }
 if (pred
	  && bpred_spec_update == spec_WB
	  && (MD_OP_FLAGS(EX_MEM_buffer.op) & F_CTRL))
	{
	  bpred_update(pred,
		       /* branch address */EX_MEM_buffer.PC,
		       /* actual target address */EX_MEM_buffer.next_PC,
		       /* taken? */EX_MEM_buffer.next_PC != (EX_MEM_buffer.PC +
						   sizeof(md_inst_t)),
		       /* pred taken? */EX_MEM_buffer.pred_PC != (EX_MEM_buffer.PC +
							sizeof(md_inst_t)),
		       /* correct pred? */EX_MEM_buffer.pred_PC == EX_MEM_buffer.next_PC,
		       /* opcode */EX_MEM_buffer.op,
		       /* dir predictor update pointer */&EX_MEM_buffer.dir_update);
	}
  /* Pass to next stage */
  MEM_WB_buffer = EX_MEM_buffer;
  MEM_WB_buffer.valid = TRUE;
  MEM_WB_buffer.ready_cycle = sim_cycle + lat;

  /* Update scoreboard with new completion time for loads */
  if (MD_OP_FLAGS(MEM_WB_buffer.op) & F_LOAD)
  {
      if (MEM_WB_buffer.out1 != NA) reg_ready_at[MEM_WB_buffer.out1] = MEM_WB_buffer.ready_cycle;
      if (MEM_WB_buffer.out2 != NA) reg_ready_at[MEM_WB_buffer.out2] = MEM_WB_buffer.ready_cycle;
  }

  EX_MEM_buffer.valid = FALSE;
...
static void execute_stage(void)
{
  ...
  if (EX_MEM_buffer.valid)
    ...
    if (!ID_EX_buffer.valid)
    return;

    inst = ID_EX_buffer.IR;
    regs.regs_PC = ID_EX_buffer.PC;
    pred_PC = ID_EX_buffer.next_PC;


     struct res_template *fu = res_get(fu_pool, MD_OP_FUCLASS(ID_EX_buffer.op));
  if (!fu)
  {
...
#include "machine.def"
	default:
      ...
      if ((pred_PC != regs.regs_NPC && pred_perfect)
	  || ((MD_OP_FLAGS(ID_EX_buffer.op) & (F_CTRL|F_DIRJMP)) == (F_CTRL|F_DIRJMP)
	      && target_PC != pred_PC && br_pred_taken))
	{
	  /* Either 1) we're simulating perfect prediction and are in a
             mis-predict state and need to patch up, or 2) We're not simulating
   ...
   if (MD_OP_FLAGS(ID_EX_buffer.op) & F_CTRL)
	sim_total_branches++;
...
if (!spec_mode)
	      ...
	      if (pred && bpred_spec_update == spec_ID)
		{
		  bpred_update(pred,
			       ...
			       /* pred taken? */pred_PC != (regs.regs_PC +
							sizeof(md_inst_t)),
			       /* correct pred? */pred_PC == regs.regs_NPC,
			       /* opcode */ID_EX_buffer.op,
			       /* predictor update ptr */&ID_EX_buffer.dir_update);
		}
	    }
  ...
  EX_MEM_buffer.valid = TRUE;
  EX_MEM_buffer.ready_cycle = sim_cycle + fu->oplat;

  if (EX_MEM_buffer.out1 != NA) reg_ready_at[EX_MEM_buffer.out1] = EX_MEM_buffer.ready_cycle;
  if (EX_MEM_buffer.out2 != NA) reg_ready_at[EX_MEM_buffer.out2] = EX_MEM_buffer.ready_cycle;
  
  ID_EX_buffer.valid = FALSE;
}
...
{
      /* get the next instruction from the IFETCH -> DISPATCH queue */
      inst = fetch_data[0].IR;
      regs.regs_PC = fetch_data[0].regs_PC;
      pred_PC = fetch_data[0].pred_PC;
      dir_update_ptr = &(fetch_data[0].dir_update);
      stack_recover_idx = fetch_data[0].stack_recover_idx;
      pseq = fetch_data[0].ptrace_seq;

      /* decode the inst */
      MD_SET_OPCODE(op, inst);
printf("DECODE: inst=0x%08x, opcode=%s, PC=0x%08lx\n", inst, MD_OP_NAME(op), regs.regs_PC);  
      /* compute default next PC */
      regs.regs_NPC = regs.regs_PC + sizeof(md_inst_t);
...
/*data hazard check*/
if ((in1 != NA && reg_ready_at[in1] > sim_cycle) ||
      (in2 != NA && reg_ready_at[in2] > sim_cycle) ||
      (in3 != NA && reg_ready_at[in3] > sim_cycle))
  {
    return;
  }

/*pass instruction to buffer*/
ID_EX_buffer.valid = TRUE;
ID_EX_buffer.IR = inst;
ID_EX_buffer.op = op;
ID_EX_buffer.PC = regs.regs_PC;
ID_EX_buffer.out1 = out1;
ID_EX_buffer.out2 = out2;
ID_EX_buffer.next_PC = regs.regs_NPC;
ID_EX_buffer.op_data[0] = regs.regs_R[in1];
ID_EX_buffer.op_data[1] = regs.regs_R[in2];
ID_EX_buffer.op_data[2] = regs.regs_R[in3];
ID_EX_buffer.pred_PC = pred_PC;
ID_EX_buffer.dir_update = *dir_update_ptr;
      /* consume instruction from IFETCH -> DISPATCH queue */
      fetch_num--;
...
static void fetch_stage(void)
{
  ...
  /* fetch only one instruction if the fetch queue is not full */
  if (fetch_num < 1) /* fetch_num can only be 0 or 1 */
    {
      /* fetch an instruction at the next predicted fetch address */
      fetch_regs_PC = fetch_pred_PC;
      ...
      if (ld_text_base <= fetch_regs_PC
          ...
          && !(fetch_regs_PC & (sizeof(md_inst_t)-1)))
        {
          ...
          /* I-cache/I-TLB miss? stall fetch until it is resolved */
          if (lat != cache_il1_lat)
            {
              fetch_delay += lat - 1;
              return; /* Stall: exit without fetching */
            }
        }
      else
        {
      ...
      if (pred)
        {
          ...
          if (!fetch_pred_PC)
            {
              /* no predicted taken target, predict not taken */
              fetch_pred_PC = fetch_regs_PC + sizeof(md_inst_t);
            }
        }
      else
        {
          /* no predictor, just predict not taken */
          fetch_pred_PC = fetch_regs_PC + sizeof(md_inst_t);
        }

      /* commit this instruction to the single-slot IFETCH -> DISPATCH queue */
      fetch_data[0].IR = inst;
      fetch_data[0].regs_PC = fetch_regs_PC;
      fetch_data[0].pred_PC = fetch_pred_PC;
      fetch_data[0].stack_recover_idx = stack_recover_idx;
      fetch_data[0].ptrace_seq = ptrace_seq++;
      
      /* for pipe trace */
      ptrace_newinst(fetch_data[0].ptrace_seq, inst, fetch_data[0].regs_PC, 0);
      ptrace_newstage(fetch_data[0].ptrace_seq, PST_IFETCH,
                      ((last_inst_missed ? PEV_CACHEMISS : 0)
                       | (last_inst_tmissed ? PEV_TLBMISS : 0)));
      last_inst_missed = FALSE;
      last_inst_tmissed = FALSE;
