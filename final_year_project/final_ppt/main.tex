...
% --- GPT-4 Data Optimization & Filtering (slide 1: text) ---
\begin{frame}{Data Optimization \& Filtering with GPT-4}
\vspace{0.3em}
\textbf{Why GPT-4?} \scite{openai2023gpt4}
\begin{itemize}
    \item Physician reports from GMC Calicut contain incomplete, colloquial, or inconsistent sentences
    \item GPT-4 acts as an \textbf{auxiliary annotation tool} to enrich and templatize raw descriptions
    \item Structured prompt: \textit{``Convert to: This image shows (region). It describes (color, texture, features.)''}
    \item All GPT-4 outputs reviewed by senior gastroenterologists for clinical accuracy
\end{itemize}
\vspace{0.6em}
...
\begin{itemize}
    ...
    \item If $S \leq 0.85$, GPT-4 regenerates until threshold is met
\end{itemize}
\end{frame}

% --- GPT-4 Data Optimization & Filtering (slide 2: figure) ---
\begin{frame}{Data Optimization \& Filtering — Sentence Selection}
\begin{figure}
\centering
\includegraphics[width=0.88\textwidth, keepaspectratio]{figures/gpt4_sentence_selection.png}
\caption{Example of sentence selection: original physician-annotated sentence $\to$ GPT-4 enhanced sentence $\to$ cosine similarity filter ($S > 0.85$). Sentences below threshold are regenerated. \scite{openai2023gpt4,reimers2019sbert}}
\end{figure}
\end{frame}
...
\vspace{0.2em}
\textbf{Plain LoRA — the limitation:} \scite{lora2022hu,plora2022liu}
\begin{itemize}
